...
@implementation UIDevice (GTHardware)
...
+ (NSString *)gt_platformString{
    NSString *platform = [self gt_platform];
...
+ (NSUInteger)gt_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)gt_cpuFrequency {
    return [self gt_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)gt_busFrequency {
    return [self gt_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)gt_ramSize {
    return [self gt_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)gt_cpuNumber {
    return [self gt_getSysInfo:HW_NCPU];
}


+ (NSUInteger)gt_totalMemoryBytes
{
    return [self gt_getSysInfo:HW_PHYSMEM];
}
