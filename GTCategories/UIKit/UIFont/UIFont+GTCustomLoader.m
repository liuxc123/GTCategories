...
@implementation UIFont (GTCustomLoader)
 ...
 */
+ (BOOL) gt_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)gt_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)gt_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)gt_registerFontFromURL:(NSURL *)fontURL {
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont gt_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont gt_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont gt_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) gt_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont gt_registerFontFromURL:fontURL];
...
+ (UIFont *) gt_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];

    return [UIFont gt_customFontWithURL:fontURL size:size];
}
