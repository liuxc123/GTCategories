...
static NSString *const GTAutocodingException = @"GTAutocodingException";

@implementation NSObject (GTAutoCoding)

+ (BOOL)supportsSecureCoding
{
    return YES;
}
+ (instancetype)gt_objectWithContentsOfFile:(NSString *)filePath
{
    //load the file
...
- (BOOL)gt_writeToFile:(NSString *)filePath atomically:(BOOL)useAuxiliaryFile
{
...
+ (NSDictionary *)gt_codableProperties
{
    //deprecated
...
- (NSDictionary *)gt_codableProperties
{
    ...
    if (!codableProperties)
    {
        ...
        {
            [(NSMutableDictionary *)codableProperties addEntriesFromDictionary:[subclass gt_codableProperties]];
...
- (NSDictionary *)gt_dictionaryRepresentation
{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    for (__unsafe_unretained NSString *key in [self gt_codableProperties])
    {
...
- (void)gt_setWithCoder:(NSCoder *)aDecoder
{
    BOOL secureAvailable = [aDecoder respondsToSelector:@selector(decodeObjectOfClass:forKey:)];
    BOOL secureSupported = [[self class] supportsSecureCoding];
    NSDictionary *properties = [self gt_codableProperties];
    for (NSString *key in properties)
    {
        ...
        if (object)
        {
            ...
            {
                [NSException raise:GTAutocodingException format:@"Expected '%@' to be a %@, but was actually a %@", key, propertyClass, [object class]];
            }
...
{
    [self gt_setWithCoder:aDecoder];
    return self;
}
#pragma clang diagnostic pop
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    for (NSString *key in [self gt_codableProperties])
    {
