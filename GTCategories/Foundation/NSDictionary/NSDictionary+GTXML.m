...
@implementation NSDictionary (GTXML)
...
- (NSString *)gt_XMLString {
    ...
    return [self  gt_XMLStringWithRootElement:nil declaration:nil];
}
- (NSString*)gt_XMLStringDefaultDeclarationWithRootElement:(NSString*)rootElement{
    return [self  gt_XMLStringWithRootElement:rootElement declaration:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"];
...
- (NSString*)gt_XMLStringWithRootElement:(NSString*)rootElement declaration:(NSString*)declaration{
    ...
    [self gt_convertNode:self withString:xml andTag:nil];
    if (rootElement) {
        [xml appendString:[NSString stringWithFormat:@"</%@>",rootElement]];
    }
...
- (void)gt_convertNode:(id)node withString:(NSMutableString *)xml andTag:(NSString *)tag{
    if ([node isKindOfClass:[NSDictionary class]] && !tag) {
        NSArray *keys = [node allKeys];
        for (NSString *key in keys) {
            [self gt_convertNode:[node objectForKey:key] withString:xml andTag:key];
        }
    }else if ([node isKindOfClass:[NSArray class]]) {
        for (id value in node) {
            [self gt_convertNode:value withString:xml andTag:tag];
        }
    }else {
        ...
        if ([node isKindOfClass:[NSString class]]) {
            ...
            [self gt_convertNode:node withString:xml andTag:nil];
        }
        [xml appendString:[NSString stringWithFormat:@"</%@>", tag]];
    }
}
...
- (NSString *)gt_plistString{
    NSString *result = [[NSString alloc] initWithData:[self gt_plistData]  encoding:NSUTF8StringEncoding];
    return result;
}
- (NSData *)gt_plistData{
