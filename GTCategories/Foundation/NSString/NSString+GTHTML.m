...
static GT_HTMLEscapeMap gt_gAsciiHTMLEscapeMap[] = {
...
static GT_HTMLEscapeMap gt_gUnicodeHTMLEscapeMap[] = {
...
static int gt_EscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
    const unichar *uchar = (const unichar*)ucharVoid;
    const GT_HTMLEscapeMap *map = (const GT_HTMLEscapeMap*)mapVoid;
    int val;
    if (*uchar > map->uchar) {
        val = 1;
...
- (NSString *)gt_stringByEscapingHTMLUsingTable:(GT_HTMLEscapeMap*)table
    ...
    for (NSUInteger i = 0; i < length; ++i) {
        GT_HTMLEscapeMap *val = bsearch(&buffer[i], table,
                                        size / sizeof(GT_HTMLEscapeMap),
                                        sizeof(GT_HTMLEscapeMap), gt_EscapeMapCompare);
...
- (NSString *)gt_stringByEscapingForHTML {
    return [self gt_stringByEscapingHTMLUsingTable:gt_gUnicodeHTMLEscapeMap
                                            ofSize:sizeof(gt_gUnicodeHTMLEscapeMap)
                                   escapingUnicode:NO];
} // gtm_stringByEscapingHTML

- (NSString *)gt_stringByEscapingForAsciiHTML {
    return [self gt_stringByEscapingHTMLUsingTable:gt_gAsciiHTMLEscapeMap
                                            ofSize:sizeof(gt_gAsciiHTMLEscapeMap)
                                   escapingUnicode:YES];
} // gtm_stringByEscapingAsciiHTML

- (NSString *)gt_stringByUnescapingFromHTML {
    ...
    do {
        ...
        if (length > 3 && length < 11) {
            if ([escapeString characterAtIndex:1] == '#') {
                ...
                // "standard" sequences
                for (unsigned i = 0; i < sizeof(gt_gAsciiHTMLEscapeMap) / sizeof(GT_HTMLEscapeMap); ++i) {
                    if ([escapeString isEqualToString:gt_gAsciiHTMLEscapeMap[i].escapeSequence]) {
                        [finalString replaceCharactersInRange:escapeRange withString:[NSString stringWithCharacters:&gt_gAsciiHTMLEscapeMap[i].uchar length:1]];
                        break;
                    }
                }
            }
...
- (NSString *)gt_stringWithNewLinesAsBRs {
    @autoreleasepool {
        // Strange New lines:
        //    Next Line, U+0085
        //    Form Feed, U+000C
...
- (NSString *)gt_stringByRemovingNewLinesAndWhitespace {
    @autoreleasepool {
...
- (NSString *)gt_stringByLinkifyingURLs {
