...
NSString *const GTZlibErrorDomain = @"se.bitba.GTZlibErrorDomain";
NSString *const GTZlibErrorInfoKey = @"zerror";

@implementation NSData (GTzlib)

static void *gt_zlibOpen()
{
    static void *libz;
...
- (NSData *)gt_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    ...
    [self gt_inflate:^(NSData *toAppend) {
...
- (NSData *)gt_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self gt_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
               error:error];
    return outData;
...
- (BOOL)gt_inflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = gt_zlibOpen();
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:GTZlibErrorDomain
                                                code:GTZlibErrorCodeInflationError
                                            userInfo:@{GTZlibErrorInfoKey : @(ret)}];
        return NO;
    }
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)gt_deflate:(void (^)(NSData *))processBlock
             error:(NSError *__autoreleasing *)error
{
    void *libz = gt_zlibOpen();
    //    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
    ...
    if (ret != Z_OK) {
        if (error) *error = [NSError errorWithDomain:GTZlibErrorDomain
                                                code:GTZlibErrorCodeDeflationError
                                            userInfo:@{GTZlibErrorInfoKey : @(ret)}];
        return NO;
    }
    Bytef *source = (Bytef *)[self bytes]; // yay
...
- (BOOL)gt_writeDeflatedToFile:(NSString *)path
                         error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = gt_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self gt_deflate:
                   ^(NSData *toAppend) {
...
static NSFileHandle *gt_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        if (!success) {
            if (error) *error = [NSError errorWithDomain:GTZlibErrorDomain
                                                    code:GTZlibErrorCodeCouldNotCreateFileError
                                                userInfo:nil];
            return nil;
        }
    }
